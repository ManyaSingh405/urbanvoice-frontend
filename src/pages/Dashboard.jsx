...
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, LineChart, Line, CartesianGrid, RadialBarChart, RadialBar } from 'recharts'
...
function IssueBadge({ type, val }) {
  return <span style={{ padding:'2px 8px', borderRadius:999, fontSize:11, fontWeight:600, background:'rgba(255,255,255,0.08)', color:'var(--text2)' }}>
          ...
          <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:20 }}>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                ...
                <Tooltip contentStyle={{background:'#1e293b',border:'1px solid #334155',borderRadius:8,color:'#f1f5f9',fontSize:12}} itemStyle={{color:'#f1f5f9'}} labelStyle={{color:'#94a3b8'}} formatter={(value, name) => [`${value}`, name]}/>
              </PieChart>
            </ResponsiveContainer>
                ...
                return (
                  <div key={auth.id}>
                    <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:5 }}>
                      <div style={{ width:22, height:22, borderRadius:'50%', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:800, background: i===0?'#facc15':i===1?'#94a3b8':i===2?'#fb923c':'rgba(255,255,255,0.08)', color: i<3?'#0f172a':'var(--text2)' }}>{i+1}</div>
        ...
        <div style={{ padding:'16px 20px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', gap:6 }}>
            {[{k:'mine',l:'My Issues'},{k:'all',l:'City Issues'}].map(t => (
              <button key={t.k} onClick={() => setTab(t.k)} style={{ padding:'6px 14px', borderRadius:8, fontSize:13, fontWeight:600, cursor:'pointer', border:'none', background: tab===t.k ? 'rgba(34,197,94,0.15)' : 'transparent', color: tab===t.k ? '#22c55e' : 'var(--text2)' }}>{t.l}</button>
            ))}
          </div>
          <button onClick={() => navigate('/map')} style={{ display:'flex', alignItems:'center', gap:5, fontSize:13, color:'#22c55e', background:'none', border:'none', cursor:'pointer', fontWeight:600 }}>
